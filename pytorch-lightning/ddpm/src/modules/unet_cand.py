...
class UNetDownBlock(nn.Module):
    ...
    def forward(self, x: Tensor, t_emb: Tensor) -> Tensor:
        ...
        # add time embedding
        out = out +  rearrange(self.t_emb_layers(t_emb), 'b c -> b c 1 1') #??
...
class UNetMidBlock(nn.Module):
    ...
    def forward(self, x: Tensor, t_emb: Tensor) -> Tensor:
        ...
        out = out +  rearrange(self.t_emb_layers[0](t_emb), 'b c -> b c 1 1') #??
        ...
        out = out +  rearrange(self.t_emb_layers[1](t_emb), 'b c -> b c 1 1')
...
class UNet(nn.Module):
    ...
    def forward(self, x: Tensor, t: Tensor)->Tensor:
        ...
        out = self.conv_in(x)
        t_emb = self.t_proj(SinusoidalPositionalEncodingBlock(self.t_emb_dim)(t).to(x.device))
